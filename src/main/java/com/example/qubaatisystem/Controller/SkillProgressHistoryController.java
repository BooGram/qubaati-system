...
public class SkillProgressHistoryController {
    ...
    public ResponseEntity<?> getById(@Valid @RequestBody IdInDTO dto) {
        return ResponseEntity.status(200).body(skillProgressHistoryService.getById(dto.getId()));
    }

    @PutMapping("/update")
    public ResponseEntity<?> update(@Valid @RequestBody SkillProgressHistoryInDTO dto) {
        skillProgressHistoryService.update(dto.getId(), dto);
    ...
    public ResponseEntity<?> delete(@Valid @RequestBody IdInDTO dto) {
        skillProgressHistoryService.delete(dto.getId());
