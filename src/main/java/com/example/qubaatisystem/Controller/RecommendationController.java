...
public class RecommendationController {
    ...
    public ResponseEntity<?> getById(@Valid @RequestBody IdInDTO dto) {
        return ResponseEntity.status(200).body(recommendationService.getById(dto.getId()));
    }

    @PutMapping("/update")
    public ResponseEntity<?> update(@Valid @RequestBody RecommendationInDTO dto) {
        recommendationService.update(dto.getId(), dto);
    ...
    public ResponseEntity<?> delete(@Valid @RequestBody IdInDTO dto) {
        recommendationService.delete(dto.getId());
