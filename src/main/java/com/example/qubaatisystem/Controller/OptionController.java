...
public class OptionController {
    ...
    public ResponseEntity<?> getById(@Valid @RequestBody IdInDTO dto) {
        return ResponseEntity.status(200).body(optionService.getById(dto.getId()));
    }

    @PutMapping("/update")
    public ResponseEntity<?> update(@Valid @RequestBody OptionInDTO dto) {
        optionService.update(dto.getId(), dto);
    ...
    public ResponseEntity<?> delete(@Valid @RequestBody IdInDTO dto) {
        optionService.delete(dto.getId());
