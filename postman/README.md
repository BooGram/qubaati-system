...
### Business-flow cleanup (body-based targets, auto analytics, honest AI feedback) — latest

- **Parent child creation creates the account only:** `POST /parents/me/children` body has **no `classroomId`** (and no `parentId`). The child starts with no classroom; `ChildCreateInDTO.classroomId` is `@Deprecated` and ignored. A **teacher** enrolls the student afterwards.
- **Teacher enrolment is body-based:** `POST /api/v1/classrooms/students/enroll` and `…/students/remove` with `{ "classroomId", "studentId" }`. Teacher from Basic Auth, must own the classroom (`/classrooms/**` = TEACHER,ADMIN → PARENT/STUDENT 403, unauth 401). Legacy path routes `…/{classroomId}/students/{studentId}/enroll|remove` are `@Deprecated` + ownership-checked.
- **Learning profile auto-initializes:** creating a student now writes an empty `LearningStyle` (primary/secondary = null, confidence = 0.0, no fake history). Skill/style values + history rows are written **only by the analytics services** after activity/mission completion. The three manual seeds (`POST /learning-styles`, `/skill-progress-history`, `/learning-style-history`) are **removed from the normal flow**; the endpoints remain **ADMIN-only** for debugging.
- **AI refine rewrites the whole activity:** `POST /api/v1/ai/activities/refine` with `{ "activityId", "instruction" }` regenerates title/description **and** questions/options/correctAnswer/points via Spring AI (e.g. "make the questions about rockets" → rocket questions), keeps status `PENDING_REVIEW`, rejects invalid/placeholder output, and **blocks if the activity is already assigned** (would corrupt attempts). Legacy `…/{activityId}/refine` is `@Deprecated`.
- **Assign-to-student is body-based:** `POST /api/v1/activity-assignments/assign-student` with `{ "activityId", "studentId", "dueDate" }`, teacher from Basic Auth (no `assignedByTeacher`, no path target ids). Legacy `/activities/{activityId}/assign/students/{studentId}` is `@Deprecated`.
- **Student start/submit/result are body-based + student-owned:** `POST /activity-assignments/start { "assignmentId" }`, `POST /activity-submissions/submit { "submissionId" }`, `POST /activity-submissions/result { "submissionId" }` — student from Basic Auth (**no `studentId` query/path**); the submission must belong to the caller (teacher acting as student → 403, unauth → 401). Legacy path routes remain and are now ownership-checked.
- **Honest AI feedback:** submissions now carry **`feedbackSource`** = `"AI"` (real Spring AI ChatClient feedback) or `"SYSTEM"` (the deterministic `Total questions… Score…` summary used when no AI provider is configured) — a template is never labelled as AI. Deterministic MCQ/TF grading is unchanged.
- **`/questions` is admin-only/debug:** no normal teacher request builds questions manually; teachers get questions through AI generation/refine. The one remaining `POST /questions` (admin) only seeds a `SHORT_ANSWER` question to exercise AI text grading, and is renamed `… [ADMIN/DEBUG seed]`.
- **Lint tool:** `postman/lint_actor_ids.py` scans folders 01–13 for actor ids in paths/bodies and for manual analytics/question creation — **0 violations** (resource targets `studentId`/`classroomId` on enroll/assign bodies and child-selection `/parents/me/children/{studentId}` are allowed).

### Tahadaw-style refactor — body-based, zero `@PathVariable`, `@AuthenticationPrincipal` (latest)

The whole API was migrated to the reference "Tahadaw" controller style:

- **Security lives in `Config`:** `SecurityConfig`, `CustomUserDetailsService`, and `SecurityOwnershipService` moved from `Security/` to `com.example.qubaatisystem.Config` (the `Security` package is gone). Still stateless HTTP Basic, BCrypt bean in `SecurityConfig`, `User implements UserDetails`. (Tahadaw uses `IF_REQUIRED` sessions + logout; we keep `STATELESS` per the no-sessions rule.)
- **`@AuthenticationPrincipal User user`:** every secured controller method takes the authenticated `User` and passes it to the ownership helper — **no `SecurityContextHolder` in controllers**. The helper is now fully `User`-based: `getCurrentTeacherId(user)`, `assertTeacherOwnsActivity(user, id)`, `assertParentOwnsChild(user, studentId)`, `assertStudentOwnsSubmission(user, id)`, etc.
- **Zero `@PathVariable`:** all **170** path variables were removed across the **29** controllers. CRUD is `POST /add`, `GET /get-all`, `POST /get` (`IdInDTO`), `PUT /update` (body carries `id`), `DELETE /delete` (`IdInDTO`). Business actions take their target ids in the body (e.g. `PATCH /activities/approve {activityId,reviewComment}`, `POST /activity-submissions/by-activity {id}`, `POST /classrooms/dashboard {id}`).
- **Actor never from the client:** profile ids (`teacherId/parentId/studentId/userId`) are derived from Basic Auth; only **resource-target** ids (`id/studentId/classroomId/activityId/assignmentId/submissionId/missionId/sessionId/…`) appear in bodies.
- **Postman fully re-pathed:** all 217 requests use the new body-based routes (3 obsolete cross-profile-id negative tests were removed because the `/me`+body design makes that vector impossible). `postman/lint_postman.py` reports **0** path-variable violations, **0** actor-id-in-body violations, and **0** requests that don't resolve to a real backend route.
